...
public class LunchMainController extends WindowAdapter 
	...
	// 키=주문번호, 값=FlagVO
	// FlagVO는 요청내용과 boolean데이터형을 가지고 있음.
	...
	private void searchOrder() {
		try {
			...
			for(int i= 0 ; i< list.size() ; i++) {
				...
				/*
				 * map에 값을 넣는 과정
				 * - map.get(orderNum) == null ? : null이라면 이전에 입력된 값이 없는 것. 때문에 false를 할당해서 아직 요청사항을 읽지 않은 상태로 만들어 준다.
				 * - ? false : map.get(orderNum).isReadFlag() : null이 아닌 상태로 이전에 값이 입력된 상태....
				 */
				if(ovo.getRequest()!=null) {
					orderNum=ovo.getOrderNum();
					
					map.put(orderNum, new FlagVO(ovo.getRequest(), map.get(orderNum) == null ? false:map.get(orderNum).isReadFlag()));
				}//end if
	...
	public void mouseClicked(MouseEvent me) {
		if(me.getSource()==lmv.getJtb()) {
			if( lmv.getJtb().getSelectedIndex() == 1 /* (주문)에서 이벤트 발생 */) {
				//  실시간으로 DB를 조회하여 주문현황을 계속 갱신
				if(threadOrdering == null) {
					threadOrdering=new Thread( this );
					threadOrdering.start();
				}//end if
				//searchOrder(); //현재까지의 주문사항을 조회
			}//end if
		}//end if
		
		if(me.getSource()==lmv.getJtOrder() && me.getButton()==MouseEvent.BUTTON3) {
			JTable jt=lmv.getJtOrder();
			 int r = jt.rowAtPoint(me.getPoint()); //마우스 포인터가 클릭되면 테이블에서 클릭한 행을 가져오는 일.
		        
			 if (r >= 0 && r < jt.getRowCount()) { //마우스 포인터가 존재하는 행 사이에서 클릭을 했을 때.
		        ...
		        /////////////////////// 요청사항추가////////////////////////////////////////////
		        // jt.getValueAt(r,1) : 테이블의 선택된 행(r)의 인덱스 1 컬럼의 값 반환.
		        // map이 주문번호를 포함하고 있고 && map의 주문번호에 해당하는 FlagVO의 readFlag가 false가 아니라면.
		        /* map에 값이 담기는 조건. */
		        //if(ovo.getRequest()!=null) {
				//	orderNum=ovo.getOrderNum();
				//	map.put(orderNum, new FlagVO(ovo.getRequest(), map.get(orderNum) == null ? false:map.get(orderNum).isReadFlag()));
		...
		switch( me. getClickCount() ) { // 더블클릭 시 이벤트 동작하기
		case  DBL_CLICK :
			
	        /////////////////////// 요청사항추가////////////////////////////////////////////
			if( me.getSource() == lmv.getJtOrder()) { //마우스 이벤트가 주문테이블에서 발생했을 때.
				JTable jt=lmv.getJtOrder();
