...
public class MultiChatServerEvt extends WindowAdapter implements ActionListener, Runnable {
	...
	@Override
	public void run() {
		// 서버 소켓을 열고 접속자 소켓을 받는 일
		try {
			server = new ServerSocket(36001); // 0 ~ 65535개의 PORT가 열릴 수 있다.
...
//				dlmTemp.addElement("[ " + ia + " / " + " ] 접속자 접속");
				...
				// 생성된 같은 이름의 소켓 객체를 여러 개 관리하기 위해 List에 추가
				listClient.add(mcsh);

				// 접속자의 메시지를 읽어들이기 위한 Thread 시작
				mcsh.start();
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource() == mcsv.getJbtOpenServer()) {
			if (threadServer == null) {
				...
				JOptionPane.showMessageDialog(mcsv, "채팅 서버가 이미 가동 중입니다.");
			}
		} // end if

		if (ae.getSource() == mcsv.getJbtCloseServer()) {
			switch (JOptionPane.showConfirmDialog(mcsv, "채팅 서버를 종료하시겠습니까?\n종료하시면 모든 접속자의 연결이 끊어집니다.")) {
