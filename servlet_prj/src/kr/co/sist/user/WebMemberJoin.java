...
public class WebMemberJoin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 사용자가 입력한 값 받기(HTML Form Control) - .getParameter() 사용.
		...
		// Parameter 값 VO에 저장하기.
		WebMemberVO wm_vo = new WebMemberVO(id, ShaUtil.shaEncoding(pass1), name,
				ShaUtil.shaEncoding(ssn1 + "-" + ssn2), gender, b_year + "-" + b_month + "-" + b_day, age, marrigeFlag,
				m_year + "-" + m_month + "-" + m_day, zipcode, addr1, addr2, greetings, request.getRemoteAddr(),
				interestlang);
		...
		out.write(
				"\t\t\t\t\tbackground: #FFFFFF url(http://localhost:8080/servlet_prj/web_member/images/header.png) no-repeat; }\t\r\n");
		out.write("\t\t\t\t\t\r\n");
		out.write("\t.span{ color: #FF0000 }\r\n");
		out.write("\t#joinContent{ position: relative;\r\n");
		out.write(
				"\t\t\twidth:773px ; height:369px ; background:#FFFFFF url(http://localhost:8080/servlet_prj/web_member/images/welcome_background.png) no-repeat; }\r\n");
		...
		try {
			// 비연결성을 해결하기 위해서.
			// DB에 Insert하기 전에 아이디를 사용중인지 DB에서 조회하기
			addWebMember(wm_vo);
			out.write("\t<div id=\"resultTitle\">\r\n");
			out.write("\t\t<span>");
			out.print(name);
			out.println("</span>님께서 입력하신 정보는 아래와 같습니다.\r\n");
			out.write("\t</div>\r\n");
			...
			out.println("아이디가 이미 사용중 입니다.<br/>");
			...
			out.println("신속히 복구하겠습니다. 사용중 불편한 점은 디잘잘에게 연락해 주세요.<br/>");
		} // end catch
