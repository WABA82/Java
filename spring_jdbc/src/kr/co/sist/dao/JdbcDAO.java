...
public class JdbcDAO {
	...
	public void insertMember(MemberVO mv) throws DataAccessException {
		...
		insertMember.append(" INSERT INTO test_like(num, name, loc, highschool, img) ").append(" VALUES(seq_reply.nextval, ?, ?, ?, ?) ");

		jt.update(insertMember.toString(), mv.getName(), mv.getLoc(), mv.getHighschool(), mv.getImg());
	}
	...
	public List<Member> selectAllMember() throws DataAccessException {
		...
		RowMapper<Member> rm = new RowMapper<Member>() {
			...
			public Member mapRow(ResultSet rs, int rowNum) throws SQLException {
				Member member = new Member(rs.getInt("num"), rs.getString("name"), rs.getString("img"));
				return member;
			}
		};
		// SQL문 작성
	...
	public DetailMember selectOneMember(int num) throws DataAccessException {
		...
		RowMapper<DetailMember> rm = new RowMapper<DetailMember>() {
			...
			public DetailMember mapRow(ResultSet rs, int rowNum) throws SQLException {
				DetailMember dm = new DetailMember(rs.getString("name"), rs.getString("img"), rs.getString("loc"), rs.getString("highschool"));
				return dm;
			}
		};
	...
	public int updateMember(MemberUpdateVO mu_vo) throws DataAccessException {
		int cnt = 0;
		String updateMember = "update test_like set name=?,highschool=?, loc=? where num=?";
		cnt = jt.update(updateMember, mu_vo.getName(), mu_vo.getHighschool(), mu_vo.getLoc(), mu_vo.getNum());
		return cnt;
	}// updateMember
	...
	public MemberImg deleteMember(int num) throws DataAccessException {
		...
		RowMapper<MemberImg> rm = new RowMapper<MemberImg>() {
			@Override
			public MemberImg mapRow(ResultSet rs, int rowNum) throws SQLException {
